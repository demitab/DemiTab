...
export const YourShareScreen = ({ eventData, profile, isDarkMode, onUpdateData, onNext }) => {
  ...
  const settlements = eventData.settlements || {}; // Fallback safety
  ...
  const handleStrategyChange = (strategy) => onUpdateData({ paymentStrategy: strategy });
  const handlePayerChange = (id) => onUpdateData({ mainPayerId: id });
          ...
          return (
            <View key={member.id} style={[styles.memberCard, themeStyles.card, (isSettled || isMainPayer) && styles.settledCard]}>
              ...
              <View style={styles.actionRow}>
                <TouchableOpacity style={[styles.actionBtn, themeStyles.input]} onPress={() => setDetailsModal({ visible: true, member })}>
                ...
                {isMainPayer ? (
                  ...
                  <TouchableOpacity disabled={!canEdit} style={[styles.actionBtn, canEdit ? themeStyles.primaryBtn : themeStyles.input]} onPress={() => { setSettleMethod('UPI'); setPaidById(null); setSettleModal({ visible: true, member }); }}>
            ...
            <View style={[styles.modalContent, themeStyles.card]}>
              ...
              <View style={styles.methodGrid}>
                {['UPI', 'CASH', 'CARD', 'OTHER'].map(method => (
                  <TouchableOpacity key={method} style={[styles.methodBtn, settleMethod === method ? styles.methodBtnActive : themeStyles.input]} onPress={() => setSettleMethod(method)}>
                ...
                <View style={{marginTop: 20}}>
                  ...
                  <ScrollView horizontal showsHorizontalScrollIndicator={false}>
                    {eventData.members.filter(m => m.id !== settleModal.member.id).map(m => (
                      <TouchableOpacity key={m.id} style={[styles.payerPill, paidById === m.id ? styles.payerPillActive : themeStyles.pillBase]} onPress={() => setPaidById(m.id)}>
