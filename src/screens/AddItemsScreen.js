...
export const AddItemsScreen = ({ eventData, profile, isDarkMode, onSaveItems }) => {
  ...
  const pickImage = async (useCamera) => {
    ...
    const result = useCamera ? await ImagePicker.launchCameraAsync({ base64: true }) : await ImagePicker.launchImageLibraryAsync({ base64: true });
    if (!result.canceled) {
      setIsScanning(true);
      try {
        ...
        const newItems = lines.map(line => {
          ...
          return { id: Math.random().toString(), name: name?.trim(), qty: parseInt(qty) || 1, price: parseFloat(price) || 0, type: 'food' };
        }).filter(i => i.name);
        setItems([...items, ...newItems]);
      } catch (e) { Alert.alert('Scan Failed', 'AI could not read receipt.'); } finally { setIsScanning(false); }
    }
  };
  ...
  useEffect(() => {
    ...
    if (needsUpdate) setItems(patchedItems);
  ...
  const addOrUpdateItem = () => {
    ...
    if (editingId) {
      ...
      setItems([{ id: Date.now().toString(), name: itemName, qty: parsedQty, price: calculatedPrice, type: itemType, assignedTo: itemType === 'food' ? eventData.members.map(m => m.id) : [], drinkCounts: {} }, ...items]);
    }
            ...
            <View style={[styles.itemCard, themeStyles.card]}>
              <View style={styles.itemHeader}>
                ...
                <View style={{alignItems: 'flex-end'}}>
                  ...
                  <TouchableOpacity style={[styles.quickToggleBadge, item.type === 'food' ? styles.typeFoodActive : styles.typeDrinkActive]} onPress={() => quickToggleType(item.id, item.type)}>
                    <Text style={styles.typeTextActive}>{item.type === 'food' ? '🍲 Food' : '🍺 Drink'}</Text>
                  </TouchableOpacity>
                </View>
              </View>
...
const styles = StyleSheet.create({ container: { flex: 1, padding: 20 }, scanWrapper: { marginBottom: 15, alignItems: 'center' }, largeScanBtn: { width: '100%', padding: 24, borderRadius: 16 }, largeScanText: { fontWeight: '900', fontSize: 20, textAlign: 'center', letterSpacing: 1 }, galleryBtn: { marginTop: 12, paddingVertical: 5 }, galleryText: { fontSize: 14, fontWeight: '600', textDecorationLine: 'underline' }, sectionDivider: { fontSize: 10, fontWeight: '800', color: '#9CA3AF', textAlign: 'center', marginVertical: 10, letterSpacing: 1 }, manualEntryContainer: { marginBottom: 20 }, inputRow: { flexDirection: 'row', gap: 6, marginBottom: 8 }, input: { padding: 12, borderRadius: 10, borderWidth: 1, fontSize: 14 }, typeRow: { flexDirection: 'row', gap: 6 }, typeSelectBtn: { flex: 1, padding: 12, borderRadius: 10, borderWidth: 1, alignItems: 'center' }, typeSelectText: { fontWeight: '700' }, typeFoodActive: { backgroundColor: '#F59E0B', borderColor: '#F59E0B' }, typeDrinkActive: { backgroundColor: '#8B5CF6', borderColor: '#8B5CF6' }, typeTextActive: { color: '#fff', fontWeight: '800' }, addBtn: { flex: 1.5, paddingHorizontal: 16, borderRadius: 10, justifyContent: 'center', alignItems: 'center' }, updateBtn: { backgroundColor: '#5BC5A7' }, addBtnText: { fontWeight: '900', fontSize: 16 }, itemCard: { borderRadius: 12, marginBottom: 16, borderWidth: 1, overflow: 'hidden' }, itemHeader: { flexDirection: 'row', justifyContent: 'space-between', padding: 16 }, itemText: { fontSize: 18, fontWeight: '800', marginBottom: 4 }, itemAmount: { fontSize: 18, fontWeight: '900', marginBottom: 4 }, quickToggleBadge: { paddingHorizontal: 10, paddingVertical: 4, borderRadius: 8 }, assignmentSection: { borderTopWidth: 1, padding: 16, backgroundColor: 'rgba(0,0,0,0.02)' }, assignmentHeader: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }, assignmentTitle: { fontSize: 13, fontWeight: '800', textTransform: 'uppercase', letterSpacing: 0.5 }, selectAllText: { color: '#3B82F6', fontWeight: '700', fontSize: 13 }, pillContainer: { flexDirection: 'row', flexWrap: 'wrap', gap: 8 }, memberPill: { paddingHorizontal: 14, paddingVertical: 8, borderRadius: 20, borderWidth: 1 }, memberPillActive: { backgroundColor: '#5BC5A7', borderColor: '#5BC5A7' }, memberPillText: { fontSize: 13, fontWeight: '600' }, memberPillTextActive: { color: '#fff', fontWeight: '800' }, drinkGridContainer: { flexDirection: 'row', flexWrap: 'wrap', marginHorizontal: -4 }, drinkGridItem: { width: '33.33%', paddingHorizontal: 4, alignItems: 'center', marginBottom: 14 }, drinkMemberName: { fontSize: 13, fontWeight: '700', marginBottom: 6, textAlign: 'center' }, stepperCompact: { flexDirection: 'row', alignItems: 'center', gap: 6 }, stepBtnCompact: { width: 28, height: 28, borderRadius: 6, borderWidth: 1, alignItems: 'center', justifyContent: 'center' }, stepBtnTextCompact: { fontSize: 16, fontWeight: '900' }, stepCountCompact: { fontSize: 14, fontWeight: '800', width: 14, textAlign: 'center' }, errorText: { color: '#EF4444', fontWeight: '800', fontSize: 12, backgroundColor: '#FEF2F2', paddingHorizontal: 8, paddingVertical: 4, borderRadius: 8 }, itemFooter: { flexDirection: 'row', justifyContent: 'space-between', padding: 12, borderTopWidth: 1 }, actionBtn: { paddingHorizontal: 10, paddingVertical: 5 }, editText: { color: '#3B82F6', fontWeight: '700', fontSize: 14 }, deleteText: { color: '#EF4444', fontWeight: '700', fontSize: 14 }, finishBtn: { backgroundColor: '#5BC5A7', marginTop: 10, padding: 18 }, finishBtnText: { color: '#fff', fontWeight: '900', fontSize: 16, textAlign: 'center' } });