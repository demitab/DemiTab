...
import { View, Text, TextInput, FlatList, StyleSheet, TouchableOpacity, ScrollView, Alert, Modal, ActivityIndicator, Platform, Linking } from 'react-native';
import * as Contacts from 'expo-contacts';
import * as Haptics from 'expo-haptics';
import QRCode from 'react-native-qrcode-svg';
import { PulseButton } from '../components/PulseButton';
import { useSafeAreaInsets } from 'react-native-safe-area-context';
import { collection, query, where, getDocs, or, doc, updateDoc } from 'firebase/firestore';
...
export const AddMembersScreen = ({ eventData, profile, isDarkMode, onSaveMembers }) => {
  const actualHostId = eventData.hostId === 'USER_ME' ? profile?.id : eventData.hostId;
  const actualHost = eventData.members.find(m => m.id === actualHostId) || { id: actualHostId, name: 'Host', phone: '' };

  const [members, setMembers] = useState(eventData.members.filter(m => m.id !== actualHostId) || []);
  ...
  const syncMembersToDB = async (updatedGuests) => {
    setMembers(updatedGuests);
    if (!eventData.id) return;
    
    const finalMembers = [actualHost, ...updatedGuests];
  ...
  useEffect(() => {
    if (eventData.members && eventData.members.length - 1 !== members.length) {
      setMembers(eventData.members.filter(m => m.id !== actualHostId));
    }
  }, [eventData.members]);
  ...
  const isDuplicate = (currentList, newName, newPhone) => {
    const cleanPhone = (phoneStr) => phoneStr ? phoneStr.replace(/\D/g, '').slice(-10) : null;
    const newPhone10 = cleanPhone(newPhone);
    
    const isHostPhone = actualHost.phone && cleanPhone(actualHost.phone) === newPhone10;
    const isHostName = actualHost.name.toLowerCase() === newName.trim().toLowerCase();
    if (isHostPhone || isHostName) return true;

    return currentList.some(m => {
      const existingPhone10 = cleanPhone(m.phone);
      if (newPhone10 && existingPhone10 && newPhone10 === existingPhone10) return true;
      return m.name.toLowerCase() === newName.trim().toLowerCase();
    });
  };

  const addManual = () => {
    if (!name.trim()) return;
    if (isDuplicate(members, name.trim(), '')) {
      Alert.alert('Notice', 'Already Added in the Group');
    } else {
      syncMembersToDB([{ name: name.trim(), phone: '', id: Date.now().toString() + Math.random() }, ...members]);
  ...
  const removeMember = (id) => {
    ...
    syncMembersToDB(members.filter(m => m.id !== id));
  };
  ...
  const submitSelectedContacts = () => {
    Haptics.notificationAsync(Haptics.NotificationFeedbackType.Success);
    let duplicates = 0;
    let newlyAddedPhones = [];
    let currentList = [...members];

    phonebook.forEach(contact => {
      if (selectedContacts.has(contact.id)) {
        if (isDuplicate(currentList, contact.name, contact.phone)) {
          duplicates++;
        } else {
          const phone10 = contact.phone ? contact.phone.replace(/\D/g, '').slice(-10) : '';
          currentList = [{ name: contact.name, phone: phone10, id: Date.now().toString() + Math.random() }, ...currentList];
          if (phone10) newlyAddedPhones.push(phone10);
        }
      }
    });

    syncMembersToDB(currentList);
    setShowContacts(false);

    if (duplicates > 0) Alert.alert('Notice', 'Some selected contacts were already in the group.');

    if (newlyAddedPhones.length > 0) {
      setTimeout(() => {
        Alert.alert(
          'Invite to DemiTab?',
          'Would you like to send a quick text to the friends you just added so they can track their share?',
          [
            { text: 'Skip', style: 'cancel' },
            { 
              text: 'Send SMS', 
              onPress: () => {
                const phoneString = newlyAddedPhones.join(',');
                const separator = Platform.OS === 'ios' ? '&' : '?';
                const inviteLink = "https://demitab-admin.vercel.app/download"; 
                const message = `Hey! I added you to the bill for '${eventData?.eventName || 'our event'}' on DemiTab. Download the app to check your share: ${inviteLink}`;
                
                Linking.openURL(`sms:${phoneString}${separator}body=${encodeURIComponent(message)}`).catch(() => {
                  Alert.alert("Error", "Could not open your SMS app.");
                });
              }
            }
          ]
      ...
      <FlatList 
        ...
        ListHeaderComponent={
          <View style={[styles.memberRow, themeStyles.hostRow]}>
            <View>
              <Text style={[styles.memberName, themeStyles.text]}>{actualHost.name} (Host)</Text>
              {actualHost.phone ? <Text style={[styles.memberPhone, themeStyles.subText]}>{actualHost.phone}</Text> : null}
            </View>
...
const styles = StyleSheet.create({ 
  ...
  addBtnText: { fontWeight: '900', fontSize: 24 }, 
  qrBtnInline: { paddingHorizontal: 15, borderRadius: 12, justifyContent: 'center', alignItems: 'center' }, 
  qrBtnInlineText: { fontWeight: '800', fontSize: 14 }, 
  quickContactsContainer: { marginBottom: 10 }, 
  ...
  hostBadge: { paddingHorizontal: 10, paddingVertical: 4, borderRadius: 12 }, 
  ...
  closeText: { color: '#EF4444', fontWeight: '700', fontSize: 16 }, 
  searchInput: { margin: 15, padding: 15 }, 
  contactRow: { flexDirection: 'row', alignItems: 'center', padding: 15, borderBottomWidth: 1 }, 
  contactAvatar: { width: 40, height: 40, borderRadius: 20, backgroundColor: '#E5E7EB', alignItems: 'center', justifyContent: 'center', marginRight: 15 }, 
  avatarText: { fontWeight: '900', color: '#6B7280' }, 
